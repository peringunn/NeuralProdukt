...
public class UpdateTrainingSet {
    ...
    private void updateHeader(List getProdukt) throws IOException {
        File file = new File("c:/TrainingSet.arff"); //for ex foo.txt
        String content = "";

        //Bygger opp stringen for produktene
        String byggProduktString = "{";
        for (int i=0; i < getProdukt.size(); i++)
        {
            if (i<getProdukt.size()-1)
                byggProduktString += "'" + getProdukt.get(i) + "',";
            else
                byggProduktString += "'" + getProdukt.get(i) + "'";
        }
        byggProduktString += "}\n";

        System.out.println(byggProduktString);
                ...
                for(int y=0;y<=checkString.length() ;y++)
                {
                    ...
                    if(compare2.equals("}"))
                    {
                             //Fjerner slutten på linjen
                                for(int x=0;x<(y);x++)
                                {
                                    checkString.charAt(i + (y-x));
                                    checkString.deleteCharAt(i + (y-x));
                                }
        ...
        content = content.replaceAll("class", "class" + byggProduktString);



        FileWriter write = new FileWriter("c:/TrainingSet.arff");
                             ...
                             String tag13, String tag14, String tag15, String tag16, String tag17, String tag18, String tag19, String produkt) throws IOException
    {
        ...
        if(tag1.equals("null") || tag2.equals("null") || tag3.equals("null") || tag4.equals("null") || tag5.equals("null") || tag6.equals("null") || tag7.equals("null") ||
                tag8.equals("null") || tag9.equals("null") || tag10.equals("null") || tag11.equals("null") || tag12.equals("null") || tag13.equals("null") ||
                tag14.equals("null") || tag15.equals("null") || tag16.equals("null") || tag17.equals("null") || tag18.equals("null") || tag19.equals("null"))
        {}
        else {
    ...
    public void readTagLoop(String datoSet) throws SQLException {
        ...
        String SQLProdUfacid = "SELECT DISTINCT (Produkt) FROM UfacidProdusert";
        String SQLProdUngerol = "SELECT DISTINCT(Produkt) FROM UngerolProdusert";
        ...
        Statement stateProdUfacid = null;
        Statement stateProdUngerol = null;
        Statement stateCountUfacid = null;
        Statement stateCountUngerol = null;
        ResultSet resultProdUfacid = null;
        ResultSet resultProdUngerol = null;
        ResultSet resultCountUfacid = null;
        ResultSet resultCountUngerol = null;

        try{
            stateProdUfacid = connection.createStatement();
            stateProdUngerol = connection.createStatement();
            resultProdUfacid = stateProdUfacid.executeQuery(SQLProdUfacid);
            resultProdUngerol = stateProdUngerol.executeQuery(SQLProdUngerol);
            //resultCountUfacid = stateCountUfacid.executeQuery(SQLCount);
            //resultCountUfacid = stateCountUngerol.executeQuery(SQLCount2);
        } catch (SQLException e)
        {
            e.printStackTrace();
        }

        //Lager en liste for å putte inn produktnavn
        List produktListe = new ArrayList();
        try{
            while (resultProdUfacid.next()) {
                produktListe.add(resultProdUfacid.getString("Produkt"));
            }
            while (resultProdUngerol.next()){
                produktListe.add(resultProdUngerol.getString("Produkt"));
            }
        } catch (SQLException e){
            e.printStackTrace();
        }
